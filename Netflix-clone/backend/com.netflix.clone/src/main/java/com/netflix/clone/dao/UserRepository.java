...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT u FROM User u WHERE LOWER(u.fullName) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(u.email) LIKE LOWER(CONCAT('%', :search, '%'))")
    Page<User> searchUsers(@Param("search") String search, Pageable pageable); // abc %abc%

    long countByRole(Role role);

    @Query("SELECT v.id FROM User u JOIN u.watchlist v WHERE u.email = :email AND v.id IN :videoIds")
    Set<Long> findWatchlistVideoIds(String email, List<Long> videoIds);

    @Query("SELECT v FROM User u JOIN u.watchlist v WHERE u.id = :id AND v.published = true AND (LOWER(v.title) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(v.description) LIKE LOWER(CONCAT('%', :search, '%')))")
    Page<Video> searchWatchlistByUserId(@Param("id") Long id, @Param("search") String search, Pageable pageable);
